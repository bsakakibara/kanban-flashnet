                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: '16px' }}>
                    {[1, 2, 3].map(i => (
                        <div
                            ...
                            style={{ backgroundColor: '#FFFFFF', border: '1px solid #D3D1C7', borderRadius: '10px', padding: '20px', height: '160px' }} />
                    ))}
                </div>
            )}
            {isError && (
                <div style={{ backgroundColor: '#FCEBEB', border: '1px solid #F09595', borderRadius: '8px', padding: '16px', color: '#A32D2D', fontSize: '13px' }}>
                    ...
                    {data?.items.map((board: Board) => {
                        ...
                        return (
                            <div
                                key={board.id}
                                onClick={() => router.push(`/boards/${board.id}`)}
                                style={{ backgroundColor: '#FFFFFF', border: '1px solid #D3D1C7', borderRadius: '10px', padding: '20px', cursor: 'pointer', display: 'flex', flexDirection: 'column', gap: '12px' }}>
                                ...
                                <div>
                                    <span style={{ fontSize: '11px', padding: '3px 8px', borderRadius: '20px', fontWeight: '500', backgroundColor: permColor.bg, color: permColor.text }}>
                                        {getPermissionLabel(board.my_permission)}
                                    </span>
                                </div>
                                <div style={{ borderTop: '1px solid #EBEBEA', paddingTop: '12px', display: 'flex', gap: '24px' }}>
                                    <div>
                                        <div style={{ fontSize: '16px', fontWeight: '600', color: '#1C1C1B' }}>
                                            {board.cards_count}
                                        </div>
                    ...
                    {data?.items.length === 0 && (
                        ...
                        >
                            ...
                            <div style={{ fontSize: '12px', color: '#888780' }}>
                                Aguarde ser adicionado a um board
                            </div>
                        </div>
                    )}
                </div>
        ...
        <>
            <div
                ...
                <div style={{ flex: 1, padding: '32px' }}>
                    <h1 style={{ fontSize: '22px', fontWeight: '700', color: '#1C1C1B', marginBottom: '24px' }}>
                        Meus Boards
                    </h1>
                    {boardsGrid}
                </div>
            ...
            <div
                className="boards-mobile"
                style={{ minHeight: '100vh', backgroundColor: '#F5F5F4', display: 'none', flexDirection: 'column' }}>
                <div style={{ backgroundColor: '#1C1C1B', padding: '12px 16px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                        ...
                        >
                            {[0, 1, 2, 3].map(i => <div
                                key={i}
                                style={{ backgroundColor: '#1C1C1B', borderRadius: '1px' }} />)}
                        </div>
                        <span style={{ fontSize: '15px', fontWeight: '600', color: '#FFFFFF' }}>
                            Kanban
                        </span>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                        ...
                        <button onClick={handleLogout}
                            ...
                            <LogOut size={14} color="#888780" />
                        </button>
                    </div>
                </div>
                <div style={{ padding: '20px 16px' }}>
                    <h1 style={{ fontSize: '18px', fontWeight: '700', color: '#1C1C1B', marginBottom: '16px' }}>
                        Meus Boards
                    </h1>
                    {boardsGrid}
                </div>
